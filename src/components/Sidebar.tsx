...
function Sidebar({ currentPage, setCurrentPage }: SidebarProps) {
  ...
  const getRoleBadgeColor = (role: string): string => {
    switch (role) {
      ...
      case "hr_staff": return "bg-blue-100 text-blue-700";
    ...
    <aside className="w-64 bg-white border-r border-gray-200 min-h-screen flex flex-col">
      ...
      {user && (
        <div className="p-4 border-t border-gray-200 bg-gray-50">
          ...
          <button
            onClick={logout}
            className="w-full flex items-center justify-center gap-2 px-3 py-2 text-sm text-red-600 bg-white border border-red-200 rounded-lg hover:bg-red-50 transition-colors"
          >
